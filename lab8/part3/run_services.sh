...
echo "Lab 8 Part 3: API Gateway Demo"
echo "==============================="
...
cd ApiGateway && mvn clean compile -q && cd ..
...
wait_for_service "http://localhost:8080/actuator/health" "API Gateway" || exit 1
...
echo "1. Open test-gateway.html in your browser"
echo "2. Or use curl: curl http://localhost:8080/api/stock/status"
echo "3. Test load balancing: curl http://localhost:8080/api/product/stock-value"
echo ""
echo "To stop all services, run: kill $EUREKA_PID $STOCK1_PID $STOCK2_PID $PRODUCT_PID $GATEWAY_PID"
echo ""
